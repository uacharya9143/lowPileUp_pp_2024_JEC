...
void MC_eta_resolution(){
#if 1
  ofstream outFile_MC("jer_sigma_rootfiles_skiplowestPtBin/MC_eta_resolution_lowPU_pp13TeV_with_newStatistics.txt");
...
gROOT->Reset();
 ...
 TFile *fIn= new TFile("phi_eta_dependent_resolution/phi_eta_resolution_JER.root");
 ...
 for(int ita=0;ita<nEta;ita++){
   ...
   cAf[ita]->Divide(3,2);
   hSigmaPlot[ita]=new TH1F(Form("hSigmaPlot_%d", ita), Form("hSigma_plot_etaReco_etaGen_%.1f_to_%.1f", etaBin[ita], etaBin[ita+1]), npt,avgPtBin );
   
   for(int ipt=0;ipt<npt; ipt++){
     // cA[ita]->cd(ipt+1);
     hpt_gen_reco[ita][ipt]=(TH1F*)fIn->Get(Form("hGenEta_RecoEta_etaBin_%d_%d",ita,ipt));
     hpt_gen_reco[ita][ipt]->SetTitle(Form("GEN_RECO_PT_Ratio_%.1f < p_{T} < %.1f;#eta_{Reco}-#eta_{Gen};",avgPtBin[ipt],avgPtBin[ipt+1]));
     ...
     hpt_gen_reco_fit[ita][ipt]->SetTitle(Form("GEN_RECO_eta_diff_fit%.1f < p_{T} < %.1f;#eta_{Reco}-#eta_{Gen};",avgPtBin[ipt],avgPtBin[ipt+1]));
     hpt_gen_reco_fit[ita][ipt]->GetXaxis()->SetRangeUser(-0.35,0.35);
     hpt_gen_reco_fit[ita][ipt]->SetMarkerStyle(20);
     hpt_gen_reco_fit[ita][ipt]->SetMarkerColor(4);
     hpt_gen_reco_fit[ita][ipt]->SetLineColor(4);
 
     ftsp1[ita][ipt]=new TF1(Form("ftsp1%d%d",ita,ipt),fgaus1,-1,1,5);
     ...
     hSigmaPlot[ita]->SetBinContent(ipt+1,hpt_gen_reco_fit[ita][ipt]->GetStdDev());
     ...
     hSigmaPlot[ita]->GetYaxis()->SetRangeUser(0.0,0.025); 
     if(ita==1||ita==2||ita==3||ita==4||ita==46||ita==47||ita==48)
       {
	 hSigmaPlot[ita]->GetYaxis()->SetRangeUser(0.0,0.07);
