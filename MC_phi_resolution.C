...
void MC_phi_resolution(){
#if 1
  ofstream outFile_MC("jer_sigma_rootfiles_skiplowestPtBin/MC_phi_resolution_lowPU_pp13TeV_with_newStatistics.txt");
...
gROOT->Reset();
 ...
 for(int ita=0;ita<nEta;ita++){
   ...
   for(int ipt=0;ipt<npt; ipt++){
     // cA[ita]->cd(ipt+1);
     hpt_gen_reco[ita][ipt]=(TH1F*)fIn->Get(Form("hGenPhi_RecoPhi_etaBin_%d_%d",ita,ipt));
     hpt_gen_reco[ita][ipt]->SetTitle(Form("GEN_RECO_PT_Ratio_%.1f < p_{T} < %.1f;#phi_{Reco}-#phi_{Gen};",avgPtBin[ipt],avgPtBin[ipt+1]));
     ...
     hpt_gen_reco_fit[ita][ipt]->SetTitle(Form("GEN_RECO_phi_diff_fit%.1f < p_{T} < %.1f;#phi_{Reco}-#phi_{Gen};",avgPtBin[ipt],avgPtBin[ipt+1]));
     hpt_gen_reco_fit[ita][ipt]->GetXaxis()->SetRangeUser(-0.2,0.2);
