...
void simu_dijet_balance_JER_JEC_Std(int infilInd=1) {
  ...
  // Save Output
  TFile *outputFile = new TFile(Form("/afs/cern.ch/user/u/uacharya/eos/cms_analaysis/JEC_analysis/simuMC_Res_output/JRA_output_%d.root",infilInd), "RECREATE");
  //hrawPt_unCor->Write();
  //hrawPt->Write();
  for (int ipt = 0; ipt < nptBin; ipt++) {
    for (int ib = 0; ib < nEta; ib++) {
