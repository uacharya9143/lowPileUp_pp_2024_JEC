...
void data_dijet_balance_JER_JEC_Std_JM0(int infilInd=1) {
  ...
  //JEC_input_file.push_back("/afs/cern.ch/user/u/uacharya/eos/cms_analaysis/JEC_analysis/JRA_step3harvest_L2Relative_AK4PF.txt");
   ...
   JEC_input_file.push_back("/afs/cern.ch/user/u/uacharya/eos/cms_analaysis/JEC_analysis/L2Residual_lowPU_pp13TeV_with_newStatistics.txt");
 JetCorrector JECCor(JEC_input_file);

 // vector<string> JEC_input_file_l2Res;
 //  //JEC_input_file.push_back("/afs/cern.ch/user/u/uacharya/eos/cms_analaysis/JEC_analysis/JRA_step3harvest_L2Relative_AK4PF.txt");
 //   JEC_input_file_l2Res.push_back("/afs/cern.ch/user/u/uacharya/MC_Truth_JEC/CMSSW_14_1_0/src/MC-truth-JEC/condor/Files/ParallelMCL1_L2Relative_AK4PF.txt");
 // JetCorrector JECCor(JEC_input_file_l2Res);
  ...
  for (int ipt = 0; ipt <nptBin; ipt++) {
    for (int i = 0; i < nEta; i++) {
      hAssymBins[ipt][i] = new TH1F(Form("hAssym_etaBin_%d_%d", ipt,i), Form("Asymmetry_in_Eta_Bin_%.1f_to_%.1f", etaBin[i], etaBin[i+1]), 200, -1, 1);
      hAssymBins[ipt][i] ->Sumw2();
      
      hAssymBins40[ipt][i] = new TH1F(Form("hAssym_etaBin40_%d_%d", ipt,i), Form("Asymmetry in Eta Bin %.1f to %.1f", etaBin[i], etaBin[i+1]), 200, -1, 1);
      hAssymBins40[ipt][i] ->Sumw2();
      hAssymBins80[ipt][i] = new TH1F(Form("hAssym_etaBin80_%d_%d", ipt,i), Form("Asymmetry in Eta Bin %.1f to %.1f", etaBin[i], etaBin[i+1]), 200, -1, 1);
      hAssymBins80[ipt][i] ->Sumw2();
      hAssymBins110[ipt][i] = new TH1F(Form("hAssym_etaBin110_%d_%d", ipt,i), Form("Asymmetry in Eta Bin %.1f to %.1f", etaBin[i], etaBin[i+1]), 200, -1, 1);
      hAssymBins110[ipt][i] ->Sumw2();
    }
  }
  
  for (int i = 0; i < nEta; i++) {
    hPt_Eta[i] = new TH1F(Form("hPt_Eta_%d", i), Form("MaxPt Eta Bin %.1f to %.1f", etaBin[i], etaBin[i+1]), 500, 0, 500);
    hPt_Eta13[i] = new TH1F(Form("hPt_Eta13_%d", i), Form("SecondMaxPt in Eta Bin %.1f to %.1f", etaBin[i], etaBin[i+1]), 500, 0, 500);
  }
  ...
  TH1F *hrawPt = new TH1F("hrawPt","hrawPt;Corrected_Leading_P_{T};",1000,0,500);
  ...
  for (Long64_t i = 0; i < nentries; i++) {
	...
	float phi = jtphi[j];
	phi_cor[j]=phi;
	//cout<<i<<'\t'<<j<<'\t'<<rawpt[j]<<'\t'<<pt_cor[j]<<endl;
	...
	//if((fabs(secondMaxEta)<=1.3 &&fabs(maxEta)<=1.3)){
	if(1){
  ...
  //TFile *outputFile = new TFile(Form("/afs/cern.ch/user/u/uacharya/eos/cms_analaysis/JEC_analysis/miniAOD_output/HiForestMiniAOD_JM1_v2_output_%d_4_.root", infilInd), "RECREATE");
  TFile *outputFile = new TFile(Form("/afs/cern.ch/user/u/uacharya/eos/cms_analaysis/JEC_analysis/JER_miniAOD/HiForestMiniAOD_JM0_output_%d.root", infilInd), "RECREATE");
  //hrawPt_unCor->Write();
  //hrawPt->Write();
  for (int ipt = 0; ipt < nptBin; ipt++) {
   for (int ib = 0; ib < nEta; ib++) {
