...
void MC_pT_dependent_resolution_factor(int infilInd=1) {
  ...
  TH1F* hGenPt_RecoPt[nEta][nptBin];
  ...
  for (int i = 0; i < nEta; i++) {
    for (int ipt = 0; ipt <nptBin; ipt++) {
      ...
      hGenPt[i][ipt] ->Sumw2();
      hGenPt_RecoPt[i][ipt] = new TH1F(Form("hGenpT_RecoPt_etaBin_%d_%d", i,ipt), Form("RecopT_GenpT_ratio_in_Eta_Bin_%.1f_to_%.1f", etaBin[i], etaBin[i+1]), 200, 0, 2);
      hGenPt_RecoPt[i][ipt] ->Sumw2();
    }
  }
  // const int nEta=4;
  float nRatio13;
	...
	for(int ipt=0;ipt<nptBin; ipt++){
	  if(eta >= etaBin[ib] && eta < etaBin[ib+1]){
	    if(maxRawPt_Gen >= avgPtBin[ipt] && maxRawPt_Gen < avgPtBin[ipt+1]){
	      hGenPt_RecoPt[ib][ipt]->Fill(maxRawPt_Reco/maxRawPt_Gen);
	      hGenPt_RecoPt[ib][ipt]->SetTitle("MC_gen_reco_pt_ratio;#frac{p_{T}^{Reco}}{p_{T}^{Gen}};");
